...
class make_active:
    def __init__(self,root):
        ...
        def getValue1():
            ...
            for row in cursor:
                if row[0]==int(e_acc):
                    ...
                    if not(row[1] =='C'):
                        messagebox.showinfo("showinfo","Your Account is  Active")
                        root.destroy()
        ...
        def close_win():
            ...
            s="update master set activity='{}' where account_num={}".format('A',e_acc)
            cursor.execute(s)
            messagebox.showinfo("showinfo","Your account has been activated.")
            root.destroy()


        btn=Button(self.root,text="Press to activate your account.",font=("times new roman",10,"bold"),bg="white",fg="black",padx=2,pady=6,command=close_win)
